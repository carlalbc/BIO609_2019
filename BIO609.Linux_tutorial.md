...
Once the **terminal** is open follow the commands below (this exercise is from the von Mering group / IMLS):
...
Here, we have used a **relative path** to make the *tmp* directory.  
We could have gotten the same result by using the *mkdir* command together with the following  **absolute path** : 
```sh 
#making the tmp directory using the absolute path (not now, you have already made the "tmp" folder)
mkdir ~/tmp
```
